...
function triangle(angleA, angleB, angleC) {
  let angleArr = [angleA, angleB, angleC];

  if (angleArr.some(val => val === 0) ||
    angleArr.reduce((acc, val) => acc + val) !== 180) return 'invalid';

  if (angleArr.some(val => val === 90)) {
    return 'right';
  } else if (angleArr.every(val => val < 90)) {
    return 'acute';
  }
  return 'obtuse';
}
